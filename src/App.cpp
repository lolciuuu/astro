...
App::App() :
		logger("App"), pIsDone(false), pEvent(new SDL_Event), pIcon(NULL) {
	...
	if (pScreen_w > MAX_SCREEN_WIDTH || pScreen_h > MAX_SCREEN_HEIGHT) {
		...
		logger.critical(
				"Screen resoulution is to large. Maximum is: 1920x1080 px");
...
/** Wyjscie z aplikacji */
App::~App() {
	logger.debug("Destruction app");
	delete pGame;

	logger.debug("Remove surface");
...
void App::run() {
	...
	if (SHOW_FPS)
		timerFPS = SDL_AddTimer(1000, FPS_callbackTimer, this);
#endif
...
void App::processEvent() {
	...
	while (SDL_PollEvent(pEvent)) {
		...
		if( pEvent->type == SDL_KEYUP ) {
			...
			else if ( pEvent->key.keysym.sym == SDLK_SPACE ) {
				pGame->SpaceDown();
			}
			else if ( pEvent->key.keysym.sym == SDLK_LSHIFT ) {
				pGame->pressedShift();
			}
			else if ( pEvent->key.keysym.sym == SDLK_RCTRL ) {
				pGame->pressedCtrl();
			}
			else if ( (pEvent->key.keysym.sym >= 97	&& (int) pEvent->key.keysym.sym <= 122) ) {
				pGame->pressedChar((char) pEvent->key.keysym.sym);
			}
			else if ( pEvent->key.keysym.sym == SDLK_BACKSPACE ) {
		...
		#ifdef DEBUG
		/** Awaryjne wylaczenie aplikacji. Po nacisnieciu F1 rzucany wyjatek */
		if (pEvent->type == SDL_KEYDOWN && pEvent->key.keysym.sym == SDLK_F1) {
...
void App::FPS_show() {
	SDL_WM_SetCaption(
			string("Debug [" + toString(pframeRating) + " fps]").c_str(), NULL);
	pframeRating = 0;
}
